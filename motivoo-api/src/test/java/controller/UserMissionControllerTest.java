...
import fixture.UserMissionFixture;
import lombok.extern.slf4j.Slf4j;
import sopt.org.motivoo.api.controller.mission.UserMissionController;
...
public class UserMissionControllerTest extends BaseControllerTest{
	...
	void getMissionImgPresignedUrl() throws Exception {
		...
		// then
		mockMvc.perform(get(DEFAULT_URL + "/image/{imgPrefix}", request)
			.contentType(MediaType.APPLICATION_JSON)
			.accept(MediaType.APPLICATION_JSON)
		).andDo(
			document("미션 인증 사진 PreSigned Url 조회 API 성공 Example",
				getDocumentRequest(),
				getDocumentResponse(),
				resource(
					ResourceSnippetParameters.builder()
						...
						.pathParameters(
							parameterWithName("imgPrefix").description("운동 인증 사진 디렉터리")
						)
						.responseFields(
							...
							fieldWithPath("data.url").type(STRING).description("S3 PreSigned Url"),
